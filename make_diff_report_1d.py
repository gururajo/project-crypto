...
logger = logging.getLogger("MARKET_1d")
...
def get_last24(diff):
    ...
    if exchanges.ok:
        ...
        for currency in exchanges['symbols']:
            ...
            time.sleep(0.5)
            try:
                past_tickers = requests.get("https://api.binance.com/api/v3/klines?symbol="+str(symbol)+"&interval=1d&startTime="+str(int((time.time()- (18*86400))*1000))+"&endTime="+str(int(time.time()*1000)))
            except Exception:
